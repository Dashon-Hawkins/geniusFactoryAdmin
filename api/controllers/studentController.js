...
module.exports = {
  create: function createFn(req, res) {
    ...
    return student.create(body).then(result => res.json(result)).catch(error => res.json(500, error));
  },

  find: function findFn(req, res) {
    return student.find({userId: req.session.user.id}).then(result => res.json(result)).catch(error => res.json(500, error));
  2},
  ...
  findOne: function findOneFn(req, res) {
    ...
    return student.findOne({id: id, userId: req.session.user.id}).then(result => res.json(result)).catch(error => res.json(500, error));
  },
  ...
  update: function updateFn(req, res) {
    ...
    return student.update({
      id: id,
      userId: userId
    }, body).then(result => res.json(result)).catch(error => res.json(500, error));
  ...
  delete: function deleteFn(req, res) {
    ...
    return student.delete({id: id, userId: req.session.user.id}).then(result => res.json(result)).catch(error => res.json(500, error));
  },

  login: function(req, res) {
    User.findOne({
      email: req.param('email')
    }, function foundUserStudent(err, user) {
      if (err)
  ...
  logout: function(req, res) {
    ...
    User.findOne(req.session.user.id, function foundUserStudent(err, user) {
      if (err)
        return res.negotiate(err);
      if (!user) {
  ...
  update: function(req, res) {
    User.update(req.session.user.id, req.body).exec(function(err, user) {
      if (err)
        return res.negotiate(err);
  ...
  findOne: function(req, res) {
    ...
    User.findOne({id: req.session.user.id}).populate('todos').exec(function(err, user) {
      if (err)
        return res.negotiate(err);
  ...
  userIdentityStudent: function(req, res) {
    if (req.session.user) {
    ...
    Passwords.encryptPassword({password: req.param('password')}).exec({
      ...
      success: function(result) {
        ...
        User.update({
          ...
          return res.json({username: updatedUser[0].username});
        });
      }
    });
  },
